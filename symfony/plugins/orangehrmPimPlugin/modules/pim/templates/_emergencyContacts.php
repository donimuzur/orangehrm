            ...
            <form name="frmEmpEmgContact" id="frmEmpEmgContact" method="post" action="<?php echo url_for('pim/updateEmergencyContact?empNumber=' . $empNumber); ?>">
                ...
                <fieldset>
                    ...
                    <p>
                        ...
                        <input type="button" id="btnCancelContact" class="reset" value="<?php echo __("Cancel"); ?>"/>
                    </p>
                </fieldset>
            </form>
        </div>
                ...
                <table id="emgcontact_list" class="table hover">
                    <thead>
                        <tr>
                            ...
                            <th class="check" id="chkEmgContact" style="width:2%"><input type='checkbox' id='checkAllEmgContact'/></th>
                            <?php }?>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <?php if ($emergencyContactPermissions->canDelete()) { ?>
                            <td class="check" id="chkEmgContact"></td>
                            <?php } ?>
                        ...
                        foreach ($emergencyContacts as $contact) :
                            ...
                            if ($emergencyContactPermissions->canDelete()) {
                                echo "<td class='check' id='chkEmgContact'><input type='checkbox' class='checkbox' id='checkboxEmgContact' name='chkecontactdel[]' value='" . $contact->seqno . "'/></td>";
                            }
                            ?>
                            <td class="emgContactName" valign="top">
                                <?php if ($emergencyContactPermissions->canUpdate()) { ?>
                                <a href="#emergencyContact"><?php echo $contact->name; ?></a>
                                <?php } else {
                                    echo $contact->name; 
                                }
                                ?>
                            </td>
                            <input type='hidden' class='check' id='chkEmgContact' name='chkdependentUP[]' value="<?php echo $contact->seqno; ?>"/>
...
<script type="text/javascript">
    ...
    function clearAddForm() {
        ...
        $('div#messagebarEmgContacts').hide();
    }

    function addEditLinks() {
        // called here to avoid double adding links - When in edit mode and cancel is pressed.
        removeEditLinks();
        $('#emgcontact_list tbody td.emgContactName').wrapInner('<a href="#emergencyContact"/>');
    }
    ...
    $(document).ready(function() {
        ...
        <?php  if (!$haveContacts){?>
        $(".check#chkEmgContact").hide();
        <?php } ?>

        $("#checkAllEmgContact").click(function(){
            if($("#checkAllEmgContact:checked").length > 0) {
                $("#checkboxEmgContact").prop('checked', true);
            } else {
                $("#checkboxEmgContact").prop('checked', false);
            }
            
            if($('#checkboxEmgContact:checkbox:checked').length > 0) {
                $('#delContactsBtn').removeAttr('disabled');
                $('#delContactsBtn').css('background-color','#aa4935');
                
            } else {
                $('#delContactsBtn').attr('disabled', 'disabled');
                $('#delContactsBtn').css('background-color','#DDD');
            }
        });

        $("#checkboxEmgContact").click(function() {
            $("#checkAllEmgContact").prop('checked', false);
            if(($("#checkboxEmgContact").length) == $("#checkboxEmgContact:checked").length) {
                $("#checkAllEmgContact").prop('checked', true);
            }
        ...
        $(document).on('click', '#frmEmpDelEmgContacts a', function() {
            ...
            var seqNo = row.find('input.check#chkEmgContact:first').val();
            var name = $(this).text();
            ...
            $('#emgContactlistActions').hide();
            $('#emgcontact_list .check#chkEmgContact').hide();
        ...
        $('#btnCancelContact').click(function() {
            ...
            $('#emgContactlistActions').show();
            $('#emgcontact_list .check#chkEmgContact').show();
            <?php  if ($emergencyContactPermissions->canUpdate()){?>
            addEditLinks();
            <?php }?>
            $('div#messagebarEmgContacts').hide();
        ...
        $('#delContactsBtn').click(function() {
            ...
            if (checked == 0) {
                $("#messagebarEmgContacts").attr("class", "messageBalloon_notice");
                $("#messagebarEmgContacts").text("<?php echo __js(TopLevelMessages::SELECT_RECORDS); ?>");
            } else {
