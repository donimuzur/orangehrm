...
<div id="dependent">
    <?php if ($dependentPermissions->canCreate() || ($haveDependents && $dependentPermissions->canUpdate())) { ?>
        ...
        <div class="head">
            <h1 id="headingDependent"><?php echo __('Add Dependent'); ?></h1>
        </div>
            ...
            <form name="frmEmpDependent" id="frmEmpDependent" method="post" action="<?php echo url_for('pim/updateDependent?empNumber=' . $empNumber); ?>">
                ...
                <fieldset>
                    ...
                    <p>
                        ...
                        <input type="button" id="btnCancelDependent" class="reset" value="<?php echo __("Cancel"); ?>"/>
                    </p>
                </fieldset>
            </form>
        </div>
                ...
                <table id="dependent_list" class="table hover">
                    <thead>
                        <tr><?php if ($dependentPermissions->canDelete()) { ?>
                            <th class="check" id='chkDependent' style="width:2%"><input type='checkbox' id='checkAllDependent' class="checkbox" /></th>
                            <?php } ?>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <?php if ($dependentPermissions->canDelete()) { ?>
                            <td class="check" id='chkDependent'></td>
                            <?php } ?>
                        ...
                        foreach ($dependents as $dependent) :
                            ...
                            echo "<td class='check' id='chkDependent'><input type='checkbox' class='checkbox' id='checkDependent' name='chkdependentdel[]' value='" . $dependent->seqno . "'/></td>";
                            } else {
                            ?>
                            ...
                            <td class="dependentName">
                                ...
                                <a href="#addPaneDependent"><?php echo $dependent->name; ?></a>
                                <?php
                                } else {
                                echo $dependent->name;
                                }
...
<script type="text/javascript">
    ...
    function clearAddForm() {
        ...
        $('div#messagebarDependent').hide();
    }

    function addEditLinks() {
        removeEditLinks();
        $('#dependent_list tbody td.dependentName').wrapInner('<a href="#addPaneDependent"/>');
    }
    ...
    $(document).ready(function() {
        ...
        $(".check#chkDependent").hide();
        <?php } ?>
        
        $("#checkAllDependent").click(function(){
            if($("#checkAllDependent:checked").length > 0) {
                $("#checkDependent").prop('checked', true);
            } else {
                $("#checkDependent").prop('checked', false);
            }
            
            if($('#checkDependent:checkbox:checked').length > 0) {
                $('#delDependentBtn').removeAttr('disabled');
                $('#delDependentBtn').css('background-color','#aa4935');
            } else {
                $('#delDependentBtn').attr('disabled', 'disabled');
                $('#delDependentBtn').css('background-color','#DDD');
            }
        });

        $("#checkDependent").click(function() {
            $("#checkAllDependent").prop('checked', false);
            if(($("#checkDependent").length) == $("#checkDependent:checked").length) {
                $("#checkAllDependent").prop('checked', true);
            }
        ...
        $(document).on('click', '#frmEmpDelDependents a', function() {
            $("#headingDependent").text("<?php echo __js("Edit Dependent");?>");
            var row = $(this).closest("tr");
            var seqNo = row.find('input.checkbox#checkDependent:first').val();
            var name = $(this).text();
            ...
            $('#dependentlistActions').hide();
            $('#dependent_list .check#chkDependent').hide();
        ...
        // Cancel in add pane
        $('#btnCancelDependent').click(function() {
            clearAddForm();
            $('#addPaneDependent').css('display', 'none');
            $('#dependentlistActions').show();
            $('#dependent_list .check#chkDependent').show();
            <?php if ($dependentPermissions->canUpdate()){?>
            addEditLinks();
            <?php }?>
            $('div#messagebarDependent').hide();
        ...
        $('#btnAddDependent').click(function() {
            $("#headingDependent").text("<?php echo __js("Add Dependent");?>");
            clearAddForm();
        ...
        $('#delDependentBtn').click(function() {
            ...
            if (checked == 0) {
                $('div#messagebarDependent').show();
                $("#messagebarDependent").attr('class', "messageBalloon_notice");
                $("#messagebarDependent").text('<?php echo __js(TopLevelMessages::SELECT_RECORDS); ?>');
            } else {
